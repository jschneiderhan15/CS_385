/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Jack Schneiderhan and Aleksander Sekulovski
 * Version     : 1.0
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x != nullptr){
        	if (key == x->value()) {
        	        ostringstream t_key;
        	  		t_key << key;
        	      	throw tree_exception("Attempt to insert duplicate key '" + t_key.str() +"'.");
        	}
        	y = x;
        	if(key < x->value()){
        		x = x->left;
...
private:
    ...
    size_t leaf_count(Node<K, V> *node) const {
    	...
    	if(node->left != nullptr){
    		leaves += leaf_count(node->left);
    	}
    	if(node->right != nullptr){
    		leaves += leaf_count(node->right);
    	}
    	return leaves;
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        ...
        if(node->left != nullptr || node->right != nullptr){
        	count++;
        	return count + internal_node_count(node->left) + internal_node_count(node->right);
        }
        else{
        	return 0;
        }
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return max(l_height + r_height + 3, max(l_diameter, r_diameter));
    ...
    size_t null_count(Node<K, V> *node) const {
        ...
        if(node->left == nullptr){
        	...
        	null_nodes +=  null_count(node->left);
        }
        if(node->right == nullptr){
        	null_nodes += 1;
        }
        else if(node->right != nullptr){
        	null_nodes +=  null_count(node->right);
        }
       return null_nodes;
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        levels_sum += level;
        level += 1;
        return levels_sum + sum_levels(node->left, level) + sum_levels(node->right, level);
    }
